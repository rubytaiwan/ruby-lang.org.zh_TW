...
這份文件包括兩大部份。這一部分的用意是整理從 _X_ 語言到 Ruby 的重點。第二部分則從 Ruby 的重要功能及特色著手，與其他程式語言來做比較。

h2. 重點整理: 從 _程式語言 X_ 到 Ruby
...
Ruby 有兩個常用的特色你可能沒見過，那就是 "程式區塊(blocks)" 和迭代子(iterators"。不像使用索引的迴圈(例如 C, C++ 和 pre-1.5 Java)，或是迴圈控制結構(例如 Perl 的 <tt>for (@a) {...}</tt>，或是 Python 的 <tt>for i in aList: ...</tt>)。在 Ruby 裡你會常常看到：
...
關於更多 @each@ 的資訊 (以及 @collect@, @find@, @inject@, @sort@ 等等)，請參考 @ri Enumerable@ (和 <tt>ri Enumerable#<i>func_name</i></tt>).
...
Symbols 就如同一個識別符號。一個 symbol 就代表它是"誰"了，而不是代表它是"什麼"。打開 <code>irb</code> 來看一看它們的區別：
...
讓我們來看看字串 "george"，它們的 <code>object_id</code> 並不相同。這表示它們在記憶體裡面是不同的物件。每次你建立一個新的字串，Ruby 就會分配新的記憶體空間給它。
...
譯著：任何類別都是 <code>Class</code> 類別所產生(new)出來的物件。
...
這裡的 <code>Constant</code> 是 10，但是 <code>Constant()</code> 卻是 11。
...
在 Java 裡，<code>public</code> 表示函式可以被任何人呼叫。<code>protected</code> 表示只有這個類別的實例、衍生類別的實例，以及相同 package 類別的實例可以呼叫，而 <code>private</code> 表示除了這個類別的實例之外，除了都不行呼叫。

Ruby 有一點不同，<code>public</code> 還是一樣是公開的意思。而 <code>private</code> 表示只有不指定接受者(receiver)時才可以呼叫。所以一個 private 函式的接受者一定是預設的 *self*。

<code>protected</code> 要特別注意。一個 protected 函式除了可以被一個類別或衍生類別的實例呼叫，也可以讓另一個相同類別的實例來當做接受者。
...
以上程式會輸出呼叫的細節，但是你隨意定義過這個訊息。

h3. 傳遞訊息，不是呼叫函數

一個函式呼叫(method call)其實就是一個 *訊息(message)* 送給一個物件：
...
此外 +=, *= 等只是 <code>var = var + other_var</code> 跟 <code>var = var * other_var</code> 等的縮寫，因此也不能被覆寫定義。
