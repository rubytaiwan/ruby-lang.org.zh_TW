你可以在這裡下載適合您的最新 Ruby 發佈版本。目前最新的穩定版本是 1.9.2。另外，請先看看 "Ruby 版權說明":http://www.ruby-lang.org/en/about/license.txt。

h3. Ruby 原始碼

如果您非常熟悉您的作業系統，或是您需要特別設定的環境，那麼直接使用原始碼來安裝是個不錯的方式。如果您的平台沒有編譯好的套件，那麼也必須透過原始碼來安裝。

* "Ruby 1.9.2-p0":ftp://ftp.ruby-lang.org:21//pub/ruby/1.9/ruby-1.9.2-p0.tar.gz (md5:&nbsp; 755aba44607c580fddc25e7c89260460) 穩定版本 (_推薦_)
...
除了上述由 C 語言實作的官方版 Ruby (又叫做 CRuby 或 MRI, Matz's Ruby Interpreter)，也有其他不同實作的 Ruby 環境。這些實作都以 "RubySpec":http://github.com/rubyspec/rubyspec 作為其語法的標準：

"JRuby":http://jruby.org/ 是由 "Java":http://www.java.com/zh_TW/ 實作的 Ruby，運行在高效能、支援系統執行緒及有非常多函數庫的 Java 虛擬機器(JVM)上。

"IronRuby":http://ironruby.net/ 是由 ".NET":http://www.microsoft.com/NET/ 實作的 Ruby，使用了 "Dynamic Language Runtime.":http://codeplex.com/dlr 技術。

"MacRuby":http://www.macruby.org/ 是由 "Objective-C":http://developer.apple.com/mac/library/documentation/Cocoa/Conceptual/ObjectiveC/Introduction/introObjectiveC.html 實作的 Ruby，直接運作在 Mac OS X 作業系統上。它使用了 "LLVM":http://llvm.org/ 編譯器技術。

"Ruby Enterprise Edition":http://www.rubyenterpriseedition.com/ (簡稱REE) 是個非常流行的 CRuby 分支版本，適合用來搭配 "Ruby on Rails":http://rubyonrails.org.tw 和 "Phusion Passenger":http://www.modrails.com/ 使用。

其他有趣的 Ruby 實作還包括 "Rubinuis":http://rubini.us/ 和 "MagLev":http://ruby.gemstone.com/ 。
